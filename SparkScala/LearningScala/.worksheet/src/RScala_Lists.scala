object RScala_Lists {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(65); 
  println("Welcome to the Scala worksheet");$skip(227); 
  ...
  val b = Stream(1, 1000000);System.out.println("""b  : scala.collection.immutable.Stream[Int] = """ + $show(b ));$skip(50); 
  // println(b(4))
  ...
  println("a.map" + a.flatMap(x => fn2(x)));$skip(44); 

  println(a.foldRight(0)((x, y) => x + y));$skip(93); 
  //foldLeft foldRight reduceLEft(auto Intialise) reduceRight
  println(a.reduceLeft(_ + _));$skip(77); 

  def fn1(x: Int): String = {
    if (x % 2 == 0) "Even"
    else "Odd"
  };System.out.println("""fn1: (x: Int)String""");$skip(69); 
  ...
  def fn3(y: => Int, x: Int): Unit = {
    println(x)
  };System.out.println("""fn3: (y: => Int, x: Int)Unit""");$skip(90); 

  val aList1 = List(List(1, 2, 3, 4, 5), List(6, 7, 8, 9, 10), List(11, 12, 13, 14, 15));System.out.println("""aList1  : List[List[Int]] = """ + $show(aList1 ));$skip(47); val res$0 = 

  aList1.reduceLeft(_ ++ _).reduceLeft(_ + _);System.out.println("""res0: Int = """ + $show(res$0));$skip(33); 
  ...
  val a3: List[(String, List[(String, List[Int])], String, List[(String, Int)])] =
    List(
      ...
      ("abcd", List(("abcd1", List(11, 22)), ("abcd2", List(33, 44))), "mdff", List(("sdfsdsdfd1", 55), ("dgdfgdfgd", 66))));System.out.println("""a3  : List[(String, List[(String, List[Int])], String, List[(String, Int)])] = """ + $show(a3 ));$skip(50); 

  println(a3.map(x => x._2.map(x => x._2.head)));$skip(34); val res$1 = 

  a3.map(x => x._2.head._2.head);System.out.println("""res1: List[Int] = """ + $show(res$1));$skip(70); 

  def f1(x: Int): Unit= {
    println(List(1 * x, 2 * x, 3 * x))
  };System.out.println("""f1: (x: Int)Unit""");$skip(46); 
