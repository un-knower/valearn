object OptionClass {
  ...
  println("https://danielwestheide.com/blog/2012/12/19/the-neophytes-guide-to-scala-part-5-the-option-type.html")
                                                  //> https://danielwestheide.com/blog/2012/12/19/the-neophytes-guide-to-scala-par
                                                  //| t-5-the-option-type.html
  println("https://alvinalexander.com/scala/collection-scala-flatmap-examples-map-flatten")
                                                  //> https://alvinalexander.com/scala/collection-scala-flatmap-examples-map-flatt
                                                  //| en
  println("https://dev.sortable.com/flatmap-to-option/")
                                                  //> https://dev.sortable.com/flatmap-to-option/
  /**
  ...
  val greeting2: Option[String] = None            //> greeting2  : Option[String] = None
  ...
  val absentGreeting: Option[String] = Option(null)
                                                  //> absentGreeting  : Option[String] = None
  val presentGreeting: Option[String] = Option("Hello!")
                                                  //> presentGreeting  : Option[String] = Some(Hello!)
  // Some test
  val NullGreeting: Option[String] = null         //> NullGreeting  : Option[String] = null
  ...
  val user1 = UserRepository.findById(1)          //> user1  : Option[OptionClass.User] = Some(User(1,John,Doe,32,Some(male)))
  if (user1.isDefined) {
    println(user1.get.firstName)
  } // will print "John"                          //> John
  ...
  val user = User(2, "Johanna", "Doe", 30, None)  //> user  : OptionClass.User = User(2,Johanna,Doe,30,None)
  ...
  val userp = User(2, "Johanna", "Doe", 30, None) //> userp  : OptionClass.User = User(2,Johanna,Doe,30,None)
  userp.gender match {
  ...
  val userq = User(2, "Johanna", "Doe", 30, None) //> userq  : OptionClass.User = User(2,Johanna,Doe,30,None)
  val genderq = userq.gender match {
    case Some(gender) => gender
    case None         => "not specified"
  }                                               //> genderq  : String = not specified
  println("Gender: " + genderq)                   //> Gender: not specified
  ...
  val gender = UserRepository.findById(1).map(_.gender)
                                                  //> gender  : Option[Option[String]] = Some(Some(male))
  ...
  val names: List[List[String]] =
    List(List("John", "Johanna", "Daniel"), List(), List("Doe", "Westheide"))
                                                  //> names  : List[List[String]] = List(List(John, Johanna, Daniel), List(), Lis
                                                  //| t(Doe, Westheide))
                                                  
  names.map(_.map(_.toUpperCase))                 //> res0: List[List[String]] = List(List(JOHN, JOHANNA, DANIEL), List(), List(D
                                                  //| OE, WESTHEIDE))
  
  
  names.flatMap(_.map(_.toUpperCase))             //> res1: List[String] = List(JOHN, JOHANNA, DANIEL, DOE, WESTHEIDE)
  ...
  val namesp: List[Option[String]] = List(Some("Johanna"), None, Some("Daniel"))
                                                  //> namesp  : List[Option[String]] = List(Some(Johanna), None, Some(Daniel))
  namesp.map(_.map(_.toUpperCase)) // List(Some("JOHANNA"), None, Some("DANIEL"))
                                                  //> res2: List[Option[String]] = List(Some(JOHANNA), None, Some(DANIEL))
  namesp.flatMap(xs => xs.map(_.toUpperCase)) // List("JOHANNA", "DANIEL")
                                                  //> res3: List[String] = List(JOHANNA, DANIEL)
   ...
   */
  List(1,2).map(UserRepository.findById)          //> res4: List[Option[OptionClass.User]] = List(Some(User(1,John,Doe,32,Some(m
                                                  //| ale))), Some(User(2,Johanna,Doe,30,None)))
  List(1,2).flatMap(UserRepository.findById)      //> res5: List[OptionClass.User] = List(User(1,John,Doe,32,Some(male)), User(2
                                                  //| ,Johanna,Doe,30,None))
  ...
  for {
    ...
    gender <- user.gender
  } yield gender // results in Some("male")       //> res6: Option[String] = Some(male)
  /**
  ...
  for {
    ...
    gender <- user.gender
  } yield gender                                  //> res7: Iterable[String] = List(male)
  ...
  } yield gender                                  //> res8: Iterable[String] = List(male)
  ...
  val resourceFromConfigDir: Option[Resource] = None
                                                  //> resourceFromConfigDir  : Option[OptionClass.Resource] = None
  val resourceFromClasspath: Option[Resource] = Some(Resource("I was found on the classpath"))
                                                  //> resourceFromClasspath  : Option[OptionClass.Resource] = Some(Resource(I wa
                                                  //| s found on the classpath))
  val resource = resourceFromConfigDir orElse resourceFromClasspath
                                                  //> resource  : Option[OptionClass.Resource] = Some(Resource(I was found on th
                                                  //| e classpath))
